...
main (int argc, char** argv)
{
  ...
  writer.write<pcl::PointXYZ>("./tmp/euclidean_cluster_extraction/after_filter.pcd", *cloud_filtered, false);
  ...
  { //迭代容器中的点云的索引，并且分开保存索引的点云
    ...
    std::stringstream ss;
    ss << "tmp/euclidean_cluster_extraction/cloud_cluster_" << j << ".pcd";
...
//以下就是我为了回答网友提问解决可视化除了平面以后的可视化的代码也就两行   
  ...
  *add_cloud+=*cloud_cluster;
  pcl::io::savePCDFileASCII("./tmp/euclidean_cluster_extraction/add_cloud.pcd",*add_cloud);
  }
